...
function EditOptionModal({ showModal, onCloseModal, optionName, optionData }: IEditOptionModalProps) {
  ...
  const OptionsList = useCallback(() => {
    ...
    return <OptionsModalList data={data} checked={checked} setChecked={setChecked} optionName={optionName} />;
  }, [optionData]);
  
    const onSubmit : SubmitHandler<IOptionForm> = option => {
    ...
    <Modal show={showModal} onHide={onHide} backdrop="static" centered>
      ...
      <Modal.Body>
        <div className='d-flex flex-column mb-3'>
          ...
          <OptionsList />
          <Button variant="danger" onClick={onDeleteClick} disabled={isDeleteDisabled} className="align-self-end mt-3">
            {useDelete.isLoading && <Spinner animation="border" variant="light" size="sm" className="me-2" />}
        ...
        <InputGroup className="mb-3 d-flex flex-column">
          <Form.Control 
            ...
            />
          {errors?.name && <p className='small text-danger mb-0 mt-1'>Please provide a valid {optionNameStr} name</p>}
        </InputGroup>
        <div className='d-flex justify-content-end mt-2'>
          <Button variant="danger" type="submit" disabled={isAddDisabled} className="me-2">
            {useAdd.isLoading && <Spinner animation="border" variant="light" size="sm" className="me-2" />}
